...
1. **Readiness that does not check the store.**
   - **Games** has no `/api/ready`. `/healthz` returns `{ ok: true, tick }` without touching `world.db`
     (`apps/server/src/net/httpServer.ts:596-600`).
   - **Chat** `/ready` is 200 as soon as `ctx.stats.lastSyncAt` is set (`server/app.js:77-86`). That
...
| **Games** | `/opt/openvibe.games/data/world.db`; `DB_PATH`, default `data/world.db` (`apps/server/src/config.ts:79`). No `.env.example`. | Yes: world, accounts, inventory | SQLite, the reviewed state. No PostgreSQL plan. | **No `/api/ready`.** `/healthz` returns `{ ok: true, tick }` (`apps/server/src/net/httpServer.ts:596-600`). | **No** | `ovhost backup` covers `world.db`. Drill **unsupported** (runtime side effects not reviewed). Games is `managed: false` in the inventory. |
...
| **Chat** | `/var/lib/openvibe-chat/chat.db` (`deploy/systemd/openvibe-chat.service:38-39`); `CHAT_DB_PATH`, default `./data/chat.db` (`server/config.js:34`). It also reads Live's `/opt/openvibe.live/data/sounds` (`:43`). | Yes, since 2026-09-23 02:03 UTC: the 12 chat tables (`table_authority = 'chat'`). Six staged tables are still Live's. | SQLite, the reviewed state. | `/ready` (loopback, `server/app.js:77-86`, not openvibe-shared). **503 only while `lastSyncAt` is unset.** It reports `live.failures`/`last_error`, `mirror.pending` and `events.enabled`, and ignores them for the status. | **No** (finding 1) | Drill passed 05:22. |
