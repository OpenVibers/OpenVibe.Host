...
async function resolveBackup(exec, inv, svc, dirArg) {
    ...
    if (dir) {
        ...
        if (!records.length) throw new DrillError(`no ovhost backup recorded for ${svc.id}; run \`ovhost backup ${svc.id}\` first or pass --backup <dir>`);
    ...
    for (const name of Object.keys(svc.drill.databases)) {
        ...
        files.push({ name, file, production: db.path, envName: svc.drill.databases[name], ext: path.extname(db.path) || '.db' });
    }
...
async function startSpec(exec, svc) {
    ...
    if (svc.units.length > 1 && !d.command) throw new DrillError(`${svc.id} runs ${svc.units.length} units; a drill starts one process, so set drill.command (or mark the drill unsupported)`);
    let directives = { execStart: null, rawExecStart: null, workingDirectory: null, user: null, environment: [] };
    if (svc.units.length) {
        const st = await systemd.show(exec, svc.units[0]);
        const texts = [];
    ...
    let argv = d.command;
    if (!argv) {
        if (!directives.execStart || !directives.execStart.length) throw new DrillError(`no ExecStart found for ${svc.units[0]}; set drill.command`);
        if (/[$%]/.test(directives.rawExecStart) || /^[^-:+!]*@/.test(directives.rawExecStart.split(/\s/)[0])) throw new DrillError(`${svc.units[0]} ExecStart uses substitutions or specifiers; set drill.command`);
    ...
    return { argv, cwd: directives.workingDirectory || svc.repo, unitUser: directives.user, environment, skippedEnvironment };
}
...
async function drill(ctx, id, { backup: backupArg = null, keep = false } = {}) {
    ...
    const d = svc.drill;
    if (d.compare.length && !svc.port) throw new DrillError(`${id} declares compare paths but no production port`);
    ...
    if (spec.unitUser && spec.unitUser !== svc.runAs) log(`warning: ${svc.units[0]} runs as ${spec.unitUser}, the inventory says ${svc.runAs}; the drill runs as ${svc.runAs}`);
    ...
    try {
        ...
        for (const f of bk.files) vars.db[f.name] = path.join(tmp, 'db', `${f.name}${f.ext}`);
        ...
        for (const f of bk.files) {
            ...
            log(`restoring ${f.file} → ${dest} (as ${svc.runAs})`);
            const cp = await exec.run('cp', ['--no-preserve=mode,ownership', '--', f.file, dest], { as: svc.runAs });
        ...
        for (const f of bk.files) lines.push(envLine(f.envName, vars.db[f.name]));
        const envPath = path.join(tmp, 'drill.env');
        await exec.writeFile(envPath, `${lines.join('\n')}\n`, { mode: 0o640 });
        const props = [
            'Type=exec',
            ...spec.environment.map((e) => environmentProperty(e.name, e.value)),
        ...
        for (const c of d.compare) {
            ...
            const prod = await exec.http(`http://127.0.0.1:${svc.port}${c.path}`, { timeoutMs: 10000, headers: h });
...
function markdownRow(r) {
    ...
    if (r.failure && r.failure.stage !== 'compare') checks.push(`${r.failure.stage}: ${cell(r.failure.message)}`);
