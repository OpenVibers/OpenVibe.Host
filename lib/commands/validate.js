...
async function validate(ctx, id) {
    ...
    if (!repoStat || !repoStat.isDir) add('error', 'checkout', `${svc.repo} does not exist`);
        ...
        // Release layout: <repo>/current -> <repo>/releases/<id>, root-owned so the service user
        // cannot rewrite the code it runs.
        ...
        else if (repoStat.owner !== 'root' || st.owner !== 'root') add('error', 'checkout', `${svc.repo} and its current release must be root-owned (found ${repoStat.owner}/${st.owner})`);
