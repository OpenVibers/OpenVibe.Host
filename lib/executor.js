 ...
 *   stat(path) -> { mode, uid, gid, owner, isFile, isDir, isSymlink, size, nlink, mtime } | null
 *   readdir(path) -> [{ name, isDir }] | null
 *   mkdir(path, { owner, mode }) ; createExclusive(path, content) -> bool ; pidAlive(pid) -> bool
 *   readStream(path) -> Readable ; writeStream(path, { mode }) -> Writable (creates the file
 *       exclusively: it fails if the path exists). Off-host backups only, which run as root.
 *   http(url, { timeoutMs, headers }) -> { status, body } | { status: 0, error }
 *   request(url, { method, headers, body, timeoutMs }) -> { status, body } | { status: 0, error }
 *       any method, http:// or https:// (release notifications: Network's token endpoint and Events'
 *       publish API). Headers and body may carry a secret: nothing here logs them.
 *   sqlite(db, sql, { as }) -> rows ; sqliteBackup(db, dest, { as })
 *   listeners(port) -> [{ pid, process }]
 *   kill(pid, signal) -> bool   (signal one pid; used only for a restore drill's own instance)
...
// runuser lives in /usr/sbin, which a systemd unit's PATH may leave out: the nightly backup unit ran with
// PATH=/usr/local/bin:/usr/bin:/bin and every database failed with `spawn runuser ENOENT` (2026-09-24/25).
const RUNUSER = ['/usr/sbin/runuser', '/sbin/runuser', '/usr/bin/runuser', '/bin/runuser']
    .find((p) => { try { fs.accessSync(p, fs.constants.X_OK); return true; } catch { return false; } }) || 'runuser';
...
function createSystemExecutor({ env = process.env } = {}) {
    ...
    async function stat(p) {
        try {
            ...
            return { mode: s.mode & 0o7777, uid: s.uid, gid: s.gid, owner: userById(s.uid), isFile: s.isFile(), isDir: s.isDirectory(), isSymlink: l.isSymbolicLink(), size: s.size, nlink: l.nlink, mtime: s.mtimeMs };
        } catch { return null; }
    }
    ...
    function request(url, { method = 'GET', headers = {}, body = null, timeoutMs = 5000 } = {}) {
        return new Promise((resolve) => {
            let u;
            try { u = new URL(url); } catch { resolve({ status: 0, error: 'bad URL' }); return; }
            const mod = u.protocol === 'https:' ? httpsLib : u.protocol === 'http:' ? httpLib : null;
            if (!mod) { resolve({ status: 0, error: 'unsupported protocol' }); return; }
            let req = null;
            let settled = false;
            const finish = (r) => { if (settled) return; settled = true; clearTimeout(timer); resolve(r); };
            const timer = setTimeout(() => { if (req) req.destroy(); finish({ status: 0, error: 'timeout' }); }, timeoutMs);
            const payload = body == null ? null : Buffer.from(String(body));
            try {
                req = mod.request(u, { method, headers: { ...headers, ...(payload ? { 'Content-Length': payload.length } : {}) } }, (res) => {
                    let text = '';
                    res.setEncoding('utf8');
                    res.on('data', (c) => { if (text.length < 1024 * 1024) text += c; });
                    res.on('end', () => finish({ status: res.statusCode, body: text }));
                    res.on('error', (err) => finish({ status: 0, error: err.code || 'response error' }));
                });
                req.on('error', (err) => finish({ status: 0, error: err.code || 'request error' }));
                if (payload) req.write(payload);
                req.end();
            } catch (err) {
                finish({ status: 0, error: err.code || 'request error' });
            }
        });
    }
    ...
    return {
        kind: 'system',
        readStream: (p) => fs.createReadStream(p),
        writeStream: (p, { mode = 0o600 } = {}) => fs.createWriteStream(p, { flags: 'wx', mode }),
        run,
        readFile,
        writeFile,
        removeFile,
        ...
        kill: async (pid, signal = 'SIGTERM') => {
            ...
            try { process.kill(pid, signal); return true; } catch (err) { if (err.code === 'ESRCH') return false; throw err; }
        },
