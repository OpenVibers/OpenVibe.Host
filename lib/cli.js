...
const FLAGS_WITH_VALUE = new Set(['inventory', 'to', 'limit', 'warn-days', 'variant', 'manifest', 'out', 'ready-timeout', 'wildcard-cert', 'backup']);
    ...
    try {
        ...
        case 'nginx': {
            if (pos[1] === 'tenants') {
                ...
                if (json && !opts.install) { emit({ files: r.files.map((f) => f.name), certDir: r.certDir, needCert: r.needCert, refused: r.refused }); return 0; }
                if (!opts.install) {
                    for (const f of r.files) out(`# ---- ${f.name} ----\n${f.text}`);
                } else {
                    const res = await lib.nginx.install(exec, inv, r.files, { log });
                    if (!res.changed.length) log('tenant vhosts already installed; nothing changed');
                }
