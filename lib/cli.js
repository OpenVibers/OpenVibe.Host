...
async function main(argv, { exec = lib.createSystemExecutor(), out = (s) => process.stdout.write(`${s}\n`), env = process.env, s3 = lib.offsite.createClient, browserCheck = runBrowserCheck } = {}) {
    ...
    try {
        ...
        case 'browser-watch': {
            ...
            if (json) emit(r);
            else for (const x of r.sites) out(x.action === 'checked' ? `${x.ok ? 'ok  ' : 'FAIL'} ${x.site} ${x.release || '?'}${x.failing && x.failing.length ? `; failing: ${x.failing.map((f) => `${f.check} ${f.fail}`).join(', ')} (details: ${ctx.inv.stateDir}/browser-watch/${x.site}.json)` : ''}`
