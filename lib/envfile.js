...
/**
 * The directives a restore drill needs from a unit file and its drop-ins, in systemd's order:
 * the last ExecStart/WorkingDirectory/User wins (an empty ExecStart= resets it), and Environment=
 * assignments accumulate. Returns { execStart: argv|null, rawExecStart, workingDirectory, user,
 * environment: [{ name, value }] }. The caller decides which Environment= values it may pass on —
 * ovhost never prints them.
 */
function unitDirectives(texts) {
    const out = { execStart: null, rawExecStart: null, workingDirectory: null, user: null, environment: [] };
    for (const text of texts) {
        for (const line of String(text || '').split('\n')) {
            const m = line.match(/^\s*(ExecStart|WorkingDirectory|User|Environment)=(.*)$/);
            if (!m) continue;
            const value = m[2].trim();
            if (m[1] === 'ExecStart') {
                out.rawExecStart = value || null;
                out.execStart = value ? splitAssignments(value.replace(/^[-@:+!]+/, '')) : null;
            } else if (m[1] === 'WorkingDirectory') out.workingDirectory = value.replace(/^-/, '') || null;
            else if (m[1] === 'User') out.user = value || null;
            else {
                for (const part of splitAssignments(value)) {
                    const i = part.indexOf('=');
                    if (i <= 0) continue;
                    const name = part.slice(0, i);
                    out.environment = out.environment.filter((e) => e.name !== name);
                    out.environment.push({ name, value: part.slice(i + 1) });
                }
            }
        }
    }
...
module.exports = { parseNames, parseExample, unitEnvironmentNames, unitDirectives, redactUnit, SECRET_NAME_RE };
